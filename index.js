const boxCon = document.querySelector(".boxContainer")
const heroText = document.querySelector(".heroText")
const heroImg = document.querySelector(".heroImg")
const myName = document.querySelector(".nameText")
const jobCon = document.querySelectorAll(".jobCon")
const jobTitle = document.querySelectorAll(".jobTitle")
const projectTitle = document.querySelector(".projectTitleMain")
...
for (let i = 0; i < 365; i++) {
    ...
    boxCon.appendChild(el)
}


window.addEventListener('scroll', () => {
    let offsetY = window.scrollY
    heroText.style.transform = `translateY(${offsetY * .1}px)`

    heroImg.style.transform = `translate(${-offsetY * .1}px,${offsetY * .7}px)`

    myName.style.transform = `translateX(${-offsetY * .1}px)`

    jobCon[0].style.backgroundPositionY = `${offsetY * .5}px`

    jobCon[1].style.backgroundPositionY = `${-offsetY * .5}px`
